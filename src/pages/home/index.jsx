    ...
    <div className='max-w-[1200px] mx-auto'>
          ...
          render={(item) => (
            ...
            >
              <div className='w-24 h-24 rounded-full border flex items-center justify-center border-[color:var(--color)] mb-2.5 relative before:w-24 before:h-24 before:absolute before:rounded-full before:bg-[color:var(--color)] before:-top-2.5 before:-right-2.5 before:opacity-10'>
                <img src={item.icon} alt={item.title} className='max-h-[50%]' />
              </div>
              ...
              <div className='border-t border-zinc-200 group-hover:border-zinc-300 dark:border-zinc-700 dark:group- hover:border-zinc-600 text-sm text-zinc-500 group-hover:text-black dark:group-hover:text-white grid grid-cols-2 gap-x-4 pt-4 mt-4'>
        ...
        <Slider
          ...
          render={(item) => (
            ...
            >
              ...
              <h6 className='line-clamp-1 text-lg mt-2.5 mb-1 font-semibold text-primary dark:text-white/80'>
                {item.title}
              </h6>
      ...
      >
        <Slider
          ...
          items={users}
          render={(item) => (
            <Link className='p-6 bg-white dark:bg-zinc-900 border hover:border-zinc-300 dark:border-zinc-700 dark:hover:border-zinc-600 rounded-md flex flex-col gap-y-2.5 items-center justify-center text-center  relative  before:w-[90%] before:h-full before:rounded-b before:absolute before:-bottom-1.5 before:left-1/2 before:-translate-x-1/2 before:bg-zinc-200/80 dark:before:bg-zinc-700 before:z-[-1]'>
              <img
                src={item.avatar}
                className='w-16 h-16 rounded-full object-cover'
              />
              <header>
                <h6 className='font-semibold text-primary line-clamp-1 dark:text-white/80'>
                  @{item.name}
                </h6>
                <time dateTime={item.date} className='text-xs text-zinc-500 '>
                  {timeAgo(item.date)}
